...
# Applying models to time series for evaluate
while current_time < end_time:
    ...
    if len(batch) > 0:
        ...
        if y_true.sum() > 0:
            ...
            print(f"[{current_time.strftime('%m-%d %H:%M')} - {next_time.strftime('%m-%d %H:%M')}] | Attack | F1: {current_f1:.4f}")

    current_time = next_time

f1_series = pd.Series(f1_scores)

# Plot the attack type histogram and the model's F1 score on a single chart
# Examining how F1 scores vary by attack type
...
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(15, 10), sharex=True, gridspec_kw={'height_ratios': [1, 1.2]})
...
static_results_df = pd.DataFrame({
    'time_window': time_stamps,
    'f1_score_static': f1_scores
})

# save baseline data for future evaluation
output_path = 'dataset/static_baseline_results.csv'
static_results_df.to_csv(output_path, index=False)