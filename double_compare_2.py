...
while current_time < end_time:
    ...
    if len(batch) > 0:
        ...
        ip_mapping_df = pd.DataFrame({
            'time_window': current_time,
            'Source IP': batch['Source IP'].values,
            'risk_score': y_prob_adaptive,
            'true_label': y_true.values
        })

        risky_batch = ip_mapping_df[ip_mapping_df['risk_score'] >= RISK_THRESHOLD]
        if len(risky_batch) > 0:
            high_risk_ips_log.append(risky_batch)
        ...
        if next_time - monitor_start_time >= psi_monitor_window:
            ...
            if len(curr_monitor_data) > 20:
                is_drift, current_psi = check_concept_drift(
                    base_distribution_data, curr_monitor_data, top_10_features, threshold=0.004)
                ...
                if is_drift:
                    ...
                    if len(retrain_df) > 0:
                        ...
                        if len(recent_attacks) < 20:
                            print(
                                f"[{next_time.strftime('%m-%d %H:%M')}] PSI warning, but not enough attack data to retrain (Guardrail blocked)")
                        else:
                            print(
                                f"[{next_time.strftime('%m-%d %H:%M')}] PSI: {current_psi:.4f} | Retraining Adaptive Model...")
...
if len(high_risk_ips_log) > 0:
    final_risk_log = pd.concat(high_risk_ips_log, ignore_index=True)

    os.makedirs('dataset', exist_ok=True)
    final_risk_log.to_csv('dataset/high_risk_ips_report.csv', index=False)

    top_ips = final_risk_log.groupby('Source IP').agg(
        Alert_Count=('time_window', 'count'),
        Avg_Risk_Score=('risk_score', 'mean'),
        Confirmed_Attacks=('true_label', 'sum')
    ).sort_values(by='Alert_Count', ascending=False).head(15)

    top_ips['Hit_Rate (%)'] = (top_ips['Confirmed_Attacks'] / top_ips['Alert_Count'] * 100).round(1)

    print(top_ips)

    fig_ip, ax_ip = plt.subplots(figsize=(14, 7))
    ips = top_ips.index.astype(str)
    alerts = top_ips['Alert_Count']
    confirmed = top_ips['Confirmed_Attacks']

    bars_alert = ax_ip.bar(ips, alerts, color='#1f77b4', alpha=0.8, label='Total Alerts (Predicted High Risk)')

    bars_confirm = ax_ip.bar(ips, confirmed, color='#d62728', alpha=0.9, label='Confirmed Malicious (True Positives)')

    for i, (alert, conf) in enumerate(zip(alerts, confirmed)):
        hit_rate = (conf / alert * 100) if alert > 0 else 0
        ax_ip.text(i, alert + (alert * 0.02), f'{hit_rate:.1f}%', ha='center', va='bottom', fontsize=10,
                   fontweight='bold', color='black')

    ax_ip.set_title('Top 15 High-Risk Source IPs Identified by Adaptive NIDS', fontsize=18, fontweight='bold', pad=20)
    ax_ip.set_ylabel('Number of Predictive Alerts', fontsize=13, fontweight='bold')
    ax_ip.set_xlabel('Source IP Address', fontsize=13, fontweight='bold')

    ax_ip.legend(loc='upper right', fontsize=12)
    ax_ip.grid(axis='y', linestyle='--', alpha=0.5)

    plt.xticks(rotation=45, ha='right', fontsize=11)

    ax_ip.spines['top'].set_visible(False)
    ax_ip.spines['right'].set_visible(False)
...
rolling_eval_window = pd.Timedelta('12h')
